...
describe('application boot (full shell over real adapters)', () => {
  ...
  it('renders the shell UI (top bar, sidebar, canvas, AI panel, status bar) without crashing', async () => {
    ...
    const writingButton = container.querySelector<HTMLButtonElement>('.side-item[title="写作"]');
    expect(writingButton).toBeTruthy();
    await act(async () => writingButton!.click());
    expect(container.querySelectorAll('.ai-panel > .pane-resize-handle.axis-y')).toHaveLength(3);
    const translateButton = [...container.querySelectorAll<HTMLButtonElement>('.ai-module-actions button')].find((button) => button.textContent?.includes('中译英'));
    expect(translateButton?.title).toContain('输入区中文');
    await act(async () => translateButton!.click());
    expect(container.textContent).toContain('请先在下方输入框粘贴需要处理的文本');
    expect(container.querySelector<HTMLButtonElement>('.ai-copy[title="复制消息"]')).toBeTruthy();
    ...
    expect(container.textContent).toContain('AI 上下文区');
    expect(container.textContent).toContain('AI 功能区');
    expect(container.textContent).toContain('AI 输入区');
